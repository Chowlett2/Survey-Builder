...
password_df = pd.read_csv('credentials.csv')
pw = password_df['shared_password'][0]
OpenAI_api_key = password_df['api_key'][0]

openai.proxy = None
# Set environment variable to disable proxy
...
if password:
    if check_password(password=password):
        st.success("Password is valid! You can now use the app.")
        authenticated = True
    else:
...
############ File management and data storage helper functions ######
...
def ensure_date_path_exists(download_dir, year, month, day):
    ...
    return day_path  # Return the final path for reference

def save_csv_with_uuid(base_dir, year, month, day, csv_df):
    # Ensure the directory structure exists
    target_dir = ensure_date_path_exists(base_dir, year, month, day)
    ...
    csv_df.to_csv(full_path, index=False)
...
if authenticated:
    ...
    # check that download directory exists and creates if missing
    ...
    if st.session_state.good_questions:
        ...
        # Download ZIP folder
        try:
            download_triggered = st.download_button('Download CSV',
                            csv,
                            f'{file_name}.csv',
                            'text/csv',
                            key='download-csv')
            ...
            if download_triggered:
                # Get current working datetime
                current_date = get_current_date()
                year = current_date['year']
                month = current_date['month']
                day = current_date['day']

                # Ensure directory path/create if needed
                ensure_date_path_exists(download_dir, year, month, day)

                # Create UUID and save file
                saved_path = save_csv_with_uuid(download_dir, year, month, day, df)
